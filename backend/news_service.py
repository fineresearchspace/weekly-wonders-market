"""Curated finance news service using approved RSS feeds and APIs."""
...
APPROVED_SOURCES={"Reuters","Bloomberg","Financial Times","The Wall Street Journal","CNBC","Trading Economics","MarketWatch","Yahoo Finance","Barron's","Mint","Moneycontrol","Economic Times Markets","Business Standard","RBI","U.S. Federal Reserve","SEBI","NSE","BSE"}
...
def _normalize(source:str,entry:Any)->dict[str,Any]|None:
    ...
    return {"id":hashlib.sha256((source+url).encode()).hexdigest()[:20],"source":source,"sourceUrl":url,"headline":headline,"snapshot":snapshot,"imageUrl":_image(entry),"publishedAt":_parse_date(entry),"category":_category(combined)}
...
def _fetch_trading_economics()->list[dict[str,Any]]:
    ...
    for news_type in ("markets","economy"):
        try:
            r=requests.get("https://api.tradingeconomics.com/news",params={"type":news_type,"c":key,"f":"json"},timeout=10)
            r.raise_for_status(); data=r.json()
            stories.extend(s for item in data if (s:=_normalize("Trading Economics",item)))
...
def get_curated_news(source:str|None=None,category:str|None=None,limit:int=40)->dict[str,Any]:
    selected=[source] if source else list(ENV_FEEDS.keys())+["Trading Economics"]
    ...
    for provider in selected:
        ...
        if not url:unavailable.append(provider);continue
        configured.append(provider);stories.extend(_fetch_feed(provider,url))
    ...
    return {"count":len(stories[:max(1,min(limit,100))]),"data":stories[:max(1,min(limit,100))],"configuredSources":configured,"unavailableSources":unavailable,"approvedSources":sorted(APPROVED_SOURCES)}